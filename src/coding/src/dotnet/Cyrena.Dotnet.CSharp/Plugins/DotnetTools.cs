...
namespace Cyrena.Dotnet.CSharp.Plugins
{
    public class DotnetTools
    {
        ...
        public DotnetTools(ISolutionController sln, IChatMessageService chat, IDevelopPlanService plan)
        {
            _sln = sln;
            _chat = chat;
            _plan = plan;
    ...
    [Description("The name of the extension, i.e. 'UserExtensions'.")] string name)
        ...
        public static string ReadTemplate(string name)
        {
            ...
            var resourceName = $"Cyrena.Dotnet.CSharp.Resources.templates.{name}";
