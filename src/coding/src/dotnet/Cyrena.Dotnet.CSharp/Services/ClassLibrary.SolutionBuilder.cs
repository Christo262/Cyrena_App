...
namespace Cyrena.Dotnet.CSharp.Services
{
    internal class ClassLibrarySolutionBuilder : ICodeBuilder
    {
        ...
        public string Id => ClassLibrary.Id;
        ...
        public async Task<DevelopPlan> ConfigureAsync(CyrenaKernelBuilder options)
        {
            ...
            var prompt = Resources.Read(typeof(DotnetExtension).Assembly, "Cyrena.Dotnet.CSharp.Resources.class-library-prompt.md");
        ...
        public async Task EditAsync(ChatConfiguration config, IServiceProvider services)
        {
            ...
            var rf = await dialog.ShowAsync<DotnetCsConfig>(ClassLibrary.Name, parameters, options);
