...
namespace Cyrena.Dotnet.CSharp.Services
{
    internal class MvcAppSolutionBuilder : ICodeBuilder
    {
        ...
        public async Task<DevelopPlan> ConfigureAsync(CyrenaKernelBuilder options)
        {
            ...
            project[DotnetOptions.Namespace] = csproj.RootNamespace;
            project[DotnetOptions.TargetFrameworks] = csproj.TargetFrameworks;
        ...
        public async Task EditAsync(ChatConfiguration config, IServiceProvider services)
        {
            ...
            var rf = await dialog.ShowAsync<DotnetCsConfig>(".NET MVC", parameters, options);
