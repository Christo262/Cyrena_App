...
namespace Cyrena.Dotnet.Services
{
    internal class BlazorLibrarySolutionsBuilder : ICodeBuilder
    {
        ...
        public BlazorLibrarySolutionsBuilder(IServiceProvider services, IKernelController kernel)
        {
            _services = services;
            _kernel = kernel;
        }

        public string Id => BlazorClassLibrary.Id;
        ...
        public async Task<DevelopPlan> ConfigureAsync(CyrenaKernelBuilder options)
        {
            ...
            var idxer = new BlazorLibraryProjectType();
            ...
            var prompt = Resources.Read(typeof(DotnetExtension).Assembly, "Cyrena.Dotnet.Resources.blazor-lib-prompt.md");
